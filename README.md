...
*   账号信息单一，使用学号登录存在信息泄露风险。计划加入自定义用户名并支持用户名登录，学号仅用于校验和内部管理
*   完善日志系统与日志审计系统，与其他功能模块的日志进行整合
*   设计Web端管理控制台，用于管理用户、服务、日志等
*   统一鉴权与filebrowser登录鉴权存在问题，需进一步完善
*   导航链接写死在main.py内，需优化，计划加入独立的配置文件管理
*   安全性任需验证
...
若要使用本服务保护其他应用（如 Grafana, Portainer 等），请在 Nginx Proxy Manager 中对应主机的 `Advanced` -> `Custom Nginx Configuration` 或 `Access Lists` 配置中指向本服务的验证接口：

*   **验证地址**: `http://<lab-portal-ip>:8000/verify`
*   **原理**: 当用户访问受保护服务时，NPM 会先请求 `/verify`。如果用户在 Lab Portal 未登录，将返回 401 并在网关层拦截（或重定向到登录页）。
